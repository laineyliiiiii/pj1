...
def main():
    ...
    prediction = np.dot(params_ols,delta)

    st.header("预测结果") 
    if prediction > 0 :
        st.write('预计2022年底标普500指数为 %1.1f，较年初上升 %1.1f' % (4796.56+prediction, prediction))
    elif prediction ==0:
        st.write('预计2022年底标普500指数为 4796.56，较年初保持不变' % ())
    elif prediction <0:
        st.write('预计2022年底标普500指数为 %1.1f，较年初下降 %1.1f' % (4796.56+prediction, -1*prediction))
